...
$cmd.CommandText = "SELECT cc.customer_id, cc.inv_no, cc.amount, cc.timestamps FROM customer_credit cc LEFT JOIN billing b ON cc.inv_no = b.inv_no AND cc.customer_id = b.customer_id WHERE b.inv_no IS NULL AND cc.is_active = 1 AND cc.inv_no IS NOT NULL AND cc.inv_no <> ''"
...
try {
    foreach ($row in $dt.Rows) {
        ...
        $insertCmd.CommandText = "INSERT INTO billing (inv_no, printed_inv_no, customer_id, subtotal, grand_total, credit_balance_due, balance_due, status, timestamps, inv_type, billing_type, payment_type, user_id, order_user_id, collector_user_id, po_number) VALUES (@inv, @inv, @cid, @amount, @amount, @amount, @amount, 'Credit', @date, 'Manual Credit', 'Credit', 'Credit', 1, 1, 1, '')"
